export class Heap<T> {
	...
	// Добавляем новый элемент в конец массива и "поднимаем" его вверх,
	// пока он не окажется на корректной позиции по приоритету.
	push(value: T) {
		this.heap.push(value);
		this.emerge(this.heap.length - 1);
	}

	// Удаляем и возвращаем корень (элемент с наивысшим приоритетом).
	// Последний элемент переносим в корень и "опускаем" его вниз,
	// пока он не встанет на правильное место.
	pop() {
